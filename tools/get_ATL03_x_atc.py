...
def get_ATL03_x_atc(IS2_atl03_mds, IS2_atl03_attrs, IS2_atl03_beams):
    ...
    for gtx in sorted(IS2_atl03_beams):
		...
		# add utm 33 N x/y
        val['heights']['utmx']=np.zeros_like(val['heights']['h_ph'])+np.NaN
        val['heights']['utmy']=np.zeros_like(val['heights']['h_ph'])+np.NaN
        utmx, utmy = to_utm(val['heights']['lat_ph'],val['heights']['lon_ph'])
        val['heights']['utmx']=utmx
        val['heights']['utmy']=utmy
