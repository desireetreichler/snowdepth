...
def read_HDF5_ATL03(FILENAME, ATTRIBUTES=True, VERBOSE=False):
    ...
    for gtx in IS2_atl03_beams:
        #key,val=fileID[gtx].items()   # this caused problems in 8/2021, so  I commented it - no use anyway?
        IS2_atl03_mds[gtx] = {}
...
#             IS2_atl03_attrs[gtx]['bckgrd_atlas'] = {}
            ...
            # add UTM 33N-converted x/y
            # does not work!
            #lon=IS2_atl03_attrs[gtx]['geolocation']['reference_photon_lon']
            #lat=IS2_atl03_attrs[gtx]['geolocation']['reference_photon_lat']
            #utmx, utmy = to_utm(lat,lon)
            #IS2_atl03_attrs[gtx]['geolocation']['utmx']=utmx
            #IS2_atl03_attrs[gtx]['geolocation']['utmy']=utmy
